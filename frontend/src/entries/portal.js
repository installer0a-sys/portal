...
function renderProfile(session, isPortalAdmin) {
  ...
  return `<div class="relative">
    <button id="profile-button" type="button" class="flex min-h-11 items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 text-sm transition hover:bg-slate-50" aria-expanded="false">
    ...
    <div id="profile-menu" class="absolute right-0 top-full z-50 mt-2 hidden w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-xl">
...
function renderLauncherShell(session) {
  ...
  root.innerHTML = `<div class="portal-shell-transition min-h-screen bg-slate-100">
    <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div class="mx-auto flex min-h-[72px] max-w-[1500px] items-center gap-4 px-4 sm:px-6">
      ...
      <button id="focus-app-search" type="button" class="ml-auto hidden min-h-11 min-w-64 items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 text-left text-sm text-slate-500 transition hover:bg-white lg:flex">${icon('search')}<span class="flex-1">Cari aplikasi...</span><kbd class="rounded-md border bg-white px-2 py-1 text-[10px]">Ctrl K</kbd></button>
      <button type="button" class="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600" title="Notifikasi">${icon('bell')}</button>
  ...
  root.innerHTML = `<div class="portal-shell-transition min-h-screen bg-slate-100 lg:flex">
    ...
    <section class="min-w-0 flex-1">
      <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur"><div class="flex min-h-[72px] items-center gap-2 px-4 sm:px-6">
        ...
        ${permissionEngine.isReadOnly(session, manifest.id) ? '<span class="ml-auto hidden rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700 md:inline-flex">Read only</span>' : '<span class="ml-auto"></span>'}
        <button type="button" class="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600" title="Cari">${icon('search')}</button>
        <button type="button" class="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600" title="Notifikasi">${icon('bell')}</button>
        ${renderProfile(session, isAdmin)}
      </div></header>
